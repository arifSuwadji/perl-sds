...
WHERE topup_status='W' and stock_ref.ref_type_id=4
	...
	while (my $row = $res->hash) {
		...
		if ($row->{nominal}) {	
			$url = 'sms?' . enurl({
				to => '899',
				ts => $row->{exec_ts},
				text => join(' ', 'trfunit', $msisdn, $row->{nominal}/1000, $row->{topup_qty}, $row->{pin}),
				modem => $row->{modem},
			});
		}
		unless ($row->{nominal}) {
                        $row->{topup_qty} =~ s/[\-\D]//g;
                        my $qty = $row->{topup_qty}/1000;
                        $url = 'sms?'. enurl({
                                to => '899',
                                ts => $row->{exec_ts},
                                text => join(' ', 'trfbulk', $msisdn, '0', $qty, $row->{pin}),
                                modem => $row->{modem},
                        });
		}
