...
while (1) {
	...
	my $res = $db->query(<<"EOS");
SELECT rs_outlet_id, topup_id, nominal, rs_number, topup_qty,
...
WHERE topup_status='W' and stock_ref.ref_type_id=3
...
EOS
	while (my $row = $res->hash) {
		...
		# nominal
		unless ($row->{nominal}) {
			my $cmd = "*171*6*3*3*1*".$row->{rs_outlet_id}."*".$row->{topup_qty}."*".$row->{pin}."#";
			$url = 'ussd?' . enurl({
				cmd => $cmd,
				ts => $row->{exec_ts},
				modem => $row->{modem},
