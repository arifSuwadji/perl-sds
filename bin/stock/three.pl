...
WHERE topup_status='W' and stock_ref.ref_type_id=5
...
EOS
	while (my $row = $res->hash) {
		...
		if ($row->{nominal}) {
			if($nom eq 35) {
				#cmd = *897*1*2*Nomor Tujuan*Jumlah Nominal*PIN#
				$cmd = "*897*1*2*".$msisdn."*".$row->{topup_qty}."*".$row->{pin}."#";		
			} elsif ($nom eq 50) {
				#cmd = *897*1*3*Nomor Tujuan*Jumlah Nominal*PIN#
				$cmd = "*897*1*3*".$msisdn."*".$row->{topup_qty}."*".$row->{pin}."#";
			} elsif ($nom eq 75) {
				#cmd = *897*1*4*Nomor Tujuan*Jumlah Nominal*PIN#
				$cmd = "*897*1*4*".$msisdn."*".$row->{topup_qty}."*".$row->{pin}."#";
			} elsif ($nom eq 125) {
				#cmd = *897*1*5*Nomor Tujuan*Jumlah Nominal*PIN#
				$cmd = "*897*1*5*".$msisdn."*".$row->{topup_qty}."*".$row->{pin}."#";
			}			 
		} elsif ($row->{nominal} eq 0) {	
			#cmd = *897*1*1*nomor tujuan*jumlah saldo*pin#
			$cmd = "*897*1*1*".$msisdn."*".$row->{topup_qty}."*".$row->{pin}."#";
		}
		my $url = 'ussd?' . enurl({
			cmd => $cmd,
			modem => $row->{modem},
