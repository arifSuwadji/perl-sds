...
while (<FH>) {
	...
	#outlet
	my %hash_outlet = (outlet_name => $cols[1], address => $cols[9], district => $cols[10], sub_district => $cols[14], outlet_type_id => 1);
	...
	#member
	$cols[4] =~ s/\s$//;
	my %hash_member = (member_name => $cols[4], site_id => $siteID, status => 'Active', parent_id => '');

	#spv
	$cols[6] =~ s/\s$//;
	my %hash_spv = (member_name => $cols[6], site_id => $siteID, status => 'Active', member_type => 'SPV', parent_id => '');

	#bm
	$cols[7] =~ s/\s$//;
	my %hash_bm = (member_name => $cols[6], site_id => $siteID, status => 'Active', member_type => 'BM');
	$log->info("BM ". Dumper \%hash_bm);

	#rs_type
	my $typeName = $cols[11] || 'W-SUMUT OTR1';
